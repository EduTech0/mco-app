<template>
  <q-dialog
    :model-value="informasiJadwal"
    @update:model-value="$emit('update:informasiJadwal', $event)"
    persistent
...
export default {
  name: "InformasiJadwalPage",

  props: {
    informasiJadwal: Boolean,
  ...
  setup(props, { emit }) {
    ...
    const internalJadwal = computed({
      get: () => props.informasiJadwal,
      set: (value) => emit("update:informasiJadwal", value),
    });
    ...
    // Computed property untuk mengelompokkan jadwals berdasarkan tanggal
