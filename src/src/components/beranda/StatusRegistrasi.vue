<template>
  <q-dialog
    :model-value="statusRegistrasi"
    @update:model-value="$emit('update:statusRegistrasi', $event)"
    persistent
...
export default {
  ...
  props: {
    statusRegistrasi: Boolean,
    maximizedToggle: Boolean,
  },

  setup(props, { emit }) {
    const internalRegistrasi = computed({
      get: () => props.statusRegistrasi,
      set: (value) => emit("update:statusRegistrasi", value),
    });
    const tickets = ref([]);
    ...
    onMounted(async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/daftar/ticket",
          {
            headers: {
