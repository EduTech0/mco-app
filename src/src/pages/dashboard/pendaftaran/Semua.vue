  ...
  data() {
    ...
    onMounted(async () => {
      try {
        ...
        daftar.value = response.data.data;
      } catch (error) {
    ...
    async verifikasi(row) {
      try {
        const response = await axios.put(
          `http://localhost:8000/api/daftar/verification/${row.id}`,
          {},
          {
            headers: {
    ...
    async deleteItem(row) {
      try {
        const response = await axios.delete(
          `http://localhost:8000/api/daftar/delete/${row.id}`,
          {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
